...
/** Columns for dense row view (prefer these keys from properties). */
...
function pickColumns(objects: ParsedObject[], max = 4): string[] {
  const scores = new Map<string, number>();
  for (const obj of objects.slice(0, 40)) {
    const props = obj.properties || {};
    for (const [k, v] of Object.entries(props)) {
      if (v === null || v === undefined || v === "" || k === "Name") continue;
      if (Array.isArray(v) && !v.length) continue;
      scores.set(k, (scores.get(k) || 0) + 1);
    }
  }
  const keys = Array.from(scores.keys());
  const preferred = COL_CANDIDATES.filter((k) =>
    keys.some((s) => s.toLowerCase() === k.toLowerCase())
  ).map((k) => {
    const found = keys.find((s) => s.toLowerCase() === k.toLowerCase());
    return found || k;
  });
...
export function CenterPane({
  ...
  const renderTable = (section: ParsedSection, limit?: number) => {
    const objs =
      limit != null ? section.objects.slice(0, limit) : section.objects;
    const cols = pickColumns(section.objects);
    return (
